...
tasks.register("generateBuildConfig") {
    ...
    val versionSource = resources.text.fromString(
        """
          ...
          |    override fun toString() = "SFC_MODULE ${project.name.uppercase()}: VERSION=${'$'}VERSION, MODBUS_VERSION=$modbusVersion, SFC_CORE_VERSION=${'$'}CORE_VERSION, SFC_IPC_VERSION=${'$'}IPC_VERSION, BUILD_DATE=${LocalDate.now()}"
          |}
          |
        """.trimMargin()
    )
